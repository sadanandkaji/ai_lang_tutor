...
const languages = ["English", "French", "Spanish", "German", "Chinese", "Japanese", "Hindi"]; // Add more as needed
...
export default function Dashboard() {
    ...
    const sendMessage = () => {
        ...
        setIsLoading(true);
        const userMessage = { user_id: "user123", known_lang: knownLang, target_lang: targetLang, message };
      ...
      <div className="h-screen flex flex-col items-center justify-center bg-gray-900 text-white p-4">
        <div className="w-full max-w-lg bg-gray-800 p-4 rounded-lg shadow-lg flex flex-col h-screen max-h-[90vh]">
          ...
          <div className="flex gap-4 py-2">
            ...
            <select className="p-2 bg-gray-700 text-white rounded flex-1" value={knownLang} onChange={(e) => setKnownLang(e.target.value)}>
              {languages.map((lang) => (<option key={lang} value={lang}>{lang}</option>))}
            </select>
            <div>Target Language</div>
            <select className="p-2 bg-gray-700 text-white rounded flex-1" value={targetLang} onChange={(e) => setTargetLang(e.target.value)}>
              {languages.map((lang) => (<option key={lang} value={lang}>{lang}</option>))}
            </select>
          </div>
          ...
          <div className="flex items-center gap-3 mt-2 p-2 border-t border-gray-700">
            ...
            />
            <Button  onClick={sendMessage} className="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded flex items-center gap-2" disabled={isLoading}>
              {isLoading ? "..." : "Send"}
            </Button>
          </div>
        </div>
